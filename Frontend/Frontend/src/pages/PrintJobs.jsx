...
import { PRINT_STATUS, API_BASE_URL } from '../utils/constants';
...
const PrintJobs = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchJobs, 1000); // Poll every 1 second
    ...
    const handleAction = async (jobId, action) => {
        try {
            ...
            if (action === 'print') {
                // Fallback: Open Print Preview if physical print fails
                // Assuming API_BASE_URL is accessible via env or context, or constructing relative
                // But for now, using hardcoded path based on known API structure
                const previewUrl = `${API_BASE_URL}/print/jobs/${jobId}/preview`;

                if (window.confirm("Printer not available. Open Print Preview instead?")) {
