...
CREATE TABLE IF NOT EXISTS shop (
    ...
    wifi_ssid TEXT,
    wifi_password TEXT,
    wifi_auth TEXT DEFAULT 'WPA',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

-- MIGRATION: Attempt to add columns if they don't exist (safe for existing DBs)
-- We use a simple try-catch block in JS after this exec, or ignore errors here if supported.
-- SQLite doesn't support IF NOT EXISTS for ADD COLUMN directly in standard SQL universally without scripting.
-- So we rely on the implementation below to run ALTER statements safely.
`);

try {
    const columns = ['wifi_ssid', 'wifi_password', 'wifi_auth'];
    columns.forEach(col => {
        try {
            db.prepare(`ALTER TABLE shop ADD COLUMN ${col} TEXT`).run();
        } catch (e) {
            // Ignore error if column exists
        }
    });
} catch (err) {
    console.log("Migration skipped or failed (harmless if columns exist)");
}
...
-- SEED DATA
INSERT OR IGNORE INTO print_pricing (color_mode, paper_size, price_per_page) VALUES 
('BW', 'A4', 2.0),
('COLOR', 'A4', 10.0),
('BW', 'A3', 5.0),
('COLOR', 'A3', 20.0);
