...
export const updatePrintJobPricing = ({ id, copies, price_per_page, cost }) => {
    db.prepare(`
        UPDATE print_jobs
        SET copies = @copies, price_per_page = @price_per_page, cost = @cost
        WHERE id = @id AND status = 'PENDING'
    `).run({ id, copies, price_per_page, cost });
};
